...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Measure serial port speed and data log, refered as Iperf(https://en.wikipedia.org/wiki/Iperf). Author: Jun Lai(mfarymail@gmail.com)")
    parser.add_argument('comport', type=int,
                    help='an integer for the serial port, cross-plat form supportted. In Linux system, it just supports for "/dev/ttyUSB*" devices')
    ...
    parser.add_argument('--random-max',type=float,dest='randomMax',help='the max frequency in the randomly-transmit mode. BLOCKED will aumatically be set to True')
    parser.add_argument('--random-min',type=float,dest='randomMin',help='the minimum frequency in the randomly-transmit mode. BLOCKED will aumatically be set to True')
    parser.add_argument('-z','--zero',dest='zeroBuffer',default= False,action='store_const',                          const=True, help='send new data untill serial buffer is empty, default: False')
